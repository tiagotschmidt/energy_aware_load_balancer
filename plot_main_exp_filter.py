...
def parse_logs(name: str, color: str, style: str, client_path: str, server_paths: List[str]) -> Experiment:
    ...
    for rps, group in sorted_groups:
        ...
        if expected_requests > 0 and actual_requests < (expected_requests * 0.99):
            print(f"[{name}] Drop detected at {rps} RPS (Expected: ~{expected_requests}, Got: {actual_requests}). Capping plot here.")
            break 
...
def plot_comparison(experiments: List[Experiment], output: str = "saturation_results.png", max_plot_rps: Optional[int] = None):
    ...
    # max_rps = max((s.target_rps for e in experiments for s in e.steps), default=0)
    # if max_plot_rps is not None:
...
def main():
    try:
        data = [
            ...
            parse_logs("Energy Aware - Marginal (AGGRESSIVE)", "yellow", "-o", 
                       "data/wmc/client_sift_experiment.csv", ["data/wmc/h2_energy.csv", "data/wmc/h3_energy.csv"])
        ]
