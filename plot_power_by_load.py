...
def load_and_process(client_csv, h2_csv, h3_csv):
    ...
    Extracts average power and average CPU utilization for each load phase.
    """
    try:
    ...
    # Step 3: Extract power and CPU data for each phase
    for step_idx, (start_t, end_t, rps) in enumerate(phases):
        ...
        # Change 'cpu_util' if your CSV header is named differently
...
def plot_host(df, host_label, cpu_col, power_col, output_filename, y_limits=None, x_limits=None):
    """
    Generates a scatter/line plot of Power Consumption vs. Host Load.
    """
    ...
    ax.plot(df_sorted[cpu_col], df_sorted[power_col], marker='o', color=color_power, linewidth=2.5, label=f'{host_label} Power Profile')
    ...
    #                 (row[cpu_col], row[power_col]),
    #                 textcoords="offset points", 
    #                 xytext=(0, 10), 
    #                 ha='center', fontsize=9)

    ax.legend(loc='upper left')
    plt.title(f'{host_label} - Power Consumption vs Host Load', fontsize=14)
    plt.tight_layout()
...
def plot_final():
    ...
    global_y_min = min(df["h2_power"].min(), df["h3_power"].min())
    global_y_max = max(df["h2_power"].max(), df["h3_power"].max())
