...
def load_and_process(client_csv, h2_csv, h3_csv):
    ...
    Extracts average efficiency and average CPU utilization for each load phase.
    """
    try:
    ...
    # Step 3: Extract efficiency and CPU data for each phase
    for step_idx, (start_t, end_t, rps) in enumerate(phases):
        ...
        avg_eff_h2 = h2_step["efficiency_score"].mean() if not h2_step.empty and "efficiency_score" in h2_step.columns else 0
        avg_eff_h3 = h3_step["efficiency_score"].mean() if not h3_step.empty and "efficiency_score" in h3_step.columns else 0
...
def plot_host_efficiency(df, host_label, cpu_col, eff_col, output_filename, y_limits=None, x_limits=None):
    """
    Generates a scatter/line plot of Efficiency Score vs. Host Load.
    """
    ...
    ax.set_ylabel('Efficiency Score', color=color_eff, fontsize=12)
    ...
    ax.plot(df_sorted[cpu_col], df_sorted[eff_col], marker='o', color=color_eff, linewidth=2.5, label=f'{host_label} Efficiency')
    ax.tick_params(axis='y', labelcolor=color_eff)
    ax.grid(True, linestyle='--', alpha=0.6)

    # # Annotate points with the RPS value for extra context
    # for _, row in df_sorted.iterrows():
    #     ax.annotate(f"{int(row['rps'])} RPS", 
    #                 (row[cpu_col], row[eff_col]),
    #                 textcoords="offset points", 
    #                 xytext=(0, 10), 
    #                 ha='center', fontsize=9)

    ax.legend(loc='upper left')
    plt.title(f'{host_label} - Efficiency vs Host Load', fontsize=14)
    plt.tight_layout()
...
def plot_final():
    ...
    global_y_min = min(df["h2_eff"].min(), df["h3_eff"].min())
    global_y_max = max(df["h2_eff"].max(), df["h3_eff"].max())
    y_padding = (global_y_max - global_y_min) * 0.05 if (global_y_max - global_y_min) > 0 else 0.1
    ...
    plot_host_efficiency(df, host_label="Host 2 (H2)", cpu_col="h2_cpu", eff_col="h2_eff", 
                         output_filename="h2_efficiency_vs_load.png", y_limits=y_limits, x_limits=x_limits)
    
    # Generate Image 2: Host 3
    plot_host_efficiency(df, host_label="Host 3 (H3)", cpu_col="h3_cpu", eff_col="h3_eff", 
                         output_filename="h3_efficiency_vs_load.png", y_limits=y_limits, x_limits=x_limits)
