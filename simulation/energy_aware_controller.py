...
class LeastUtilizedPolicy(LoadBalancingPolicy):
    ...
    def evaluate(self, stats: Dict[str, ServerStat], util: float, n_servers: int) -> List[str]:
        ...
        logger.info(f"LeastUtilized Evaluation | Server Scores: {[f'{s.host} (util={s.util:.1f}%)' for s in ordered]}")
        selected_servers = [s.host for s in ordered[:1]]

        if len(selected_servers) < n_servers:
            selected_servers.extend(selected_servers[-1] for _ in range(n_servers - len(selected_servers)))
...
class MarginalCostPolicy(LoadBalancingPolicy):
    ...
    def evaluate(self, stats: Dict[str, ServerStat], util: float, n_servers: int) -> List[str]:
        ...
        average_cluster_utilization = raw_total_utilization / len(available_hosts) / 100.0 if available_hosts else 0.0
        ...
        for host, _ in remainders:
            if len(selected_servers) >= n_servers: break
            selected_servers.append(host)

        while len(selected_servers) < n_servers:
            selected_servers.append(available_hosts[len(selected_servers) % len(available_hosts)])
    ...
    def evaluate(self, stats: Dict[str, ServerStat], util: float, n_servers: int) -> List[str]:
        ...
        for host, stat in stats.items():
            ...
            if self.mab_total_pulls > 1 and self.mab_counts[host][bucket] > 0:
                exploration = math.sqrt((2 * math.log(self.mab_total_pulls)) / float(self.mab_counts[host][bucket]))
        ...
        if len(ordered_hosts) < n_servers:
            ordered_hosts.extend(ordered_hosts[0] for _ in range(n_servers - len(ordered_hosts)))
...
class MyLBController:
    def __init__(self, policy: LoadBalancingPolicy, p4info_path, bmv2_json_path):
        self.policy = policy
        ...
        print("Initializing Default Forwarding Rules (Round Robin across all)...")
        default_servers = [f"h{i}" for i in range(2, 9)] 
        expanded_defaults = [default_servers[i % len(default_servers)] for i in range(NUMBER_SWITCH_TABLE_ENTRIES)]
    ...
    def install_egress_rewrite_rules(self):
        print("Installing Egress Rewrite Rules for 8 hosts...")
        for port in range(1, 9):
            smac = f"08:00:00:00:0{port}:0{port}"
    ...
    def install_return_path_rule(self):
        ...
        servers = [f"10.0.{i}.{i}" for i in range(2, 9)]
    ...
    def update_switch_tables(self, priority_hosts: List[str]):
        server_info = {
            f"h{i}": {"ip": f"10.0.{i}.{i}", "mac": f"08:00:00:00:0{i}:0{i}", "port": i} 
            for i in range(2, 9)
        }
        
        for index, hostname in enumerate(priority_hosts):
            if index >= NUMBER_SWITCH_TABLE_ENTRIES: break
...
if __name__ == "__main__":
    ...
    print("Starting Load Balancer Controller. Using Marginal-cost Energy-Aware Priority.")
    
    # active_policy = LeastUtilizedPolicy()
    # active_policy = RoundRobinPolicy()
    # active_policy = MabPolicy()

    ctrl = MyLBController(active_policy, P4INFO_FILE, JSON_FILE)